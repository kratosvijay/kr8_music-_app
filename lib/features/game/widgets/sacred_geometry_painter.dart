...
class SacredGeometryPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Outer double border of the blueprint sheet
    ...
    // === 4. MAIN OILET TRIANGULUS DIAGRAM ===
    ...
    final double invTopY = topCircleCenter.dy;
    final double invTopXDist = (invTopY - topVertex.dy).abs() / sqrt(3);
    ...
    // Draw 3 Main double Inner Circles
    ...
    // Small Node Circles at 4 Vertices
    const double nodeR = 15.0;
    ...
    _drawText(canvas, "G#/F#", invBottom, alignCenter: true, fontWeight: FontWeight.bold);

    // Other solfege / notes on the diagram
    ...
    // Note positions on the blueprint chromatic ring
    _drawText(canvas, "Db", Offset(center.dx + R * 0.28, center.dy - R * 0.52), alignCenter: true);
    _drawText(canvas, "D", Offset(center.dx + R * 0.60, center.dy - R * 0.35), alignCenter: true);
    _drawText(canvas, "Eb", Offset(center.dx + R * 0.60, center.dy + R * 0.05), alignCenter: true);
    _drawText(canvas, "E", Offset(center.dx + R * 0.62, center.dy + R * 0.36), alignCenter: true);

    _drawText(canvas, "Ab", Offset(center.dx - R * 0.62, center.dy + R * 0.36), alignCenter: true);
    _drawText(canvas, "A", Offset(center.dx - R * 0.60, center.dy + R * 0.05), alignCenter: true);
    _drawText(canvas, "Bb", Offset(center.dx - R * 0.60, center.dy - R * 0.35), alignCenter: true);
    _drawText(canvas, "B", Offset(center.dx - R * 0.28, center.dy - R * 0.52), alignCenter: true);

    // === 5. PANEL 1: CIRCLE AND RING STUDY ===
    ...
    // Arrows pointing to circles (drafting lines)
    ...
    // === 6. PANEL 2: UPRIGHT TRIANGLE DETAIL ===
  ...
  void _drawText(
    ...
    final textSpan = TextSpan(
      ...
      style: TextStyle(
        ...
        fontFamily: 'monospace', // Gives a technical blueprint look
      ),
    );
